...
视频**同时存了两份**：一份在 GitHub 上，让这个页面能内嵌播放；一份在我们自己的存储上，供画廊调用。两份都需要，因为 X 会轮换 `video.twimg.com` 的地址，只靠外链迟早会让整个索引集体失效。存的是 X 自己发布的那份编码，不二次剪辑、不换配乐、不改写文案。我们不会把任何人的作品说成是自己的。
